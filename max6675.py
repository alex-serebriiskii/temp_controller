...
class MAX6675(object):
    ...
    '''
    def __init__(self, clock_pin, data_pin, units = "c"):
        '''Initialize Soft (Bitbang) SPI bus
        Parameters:
        - cs_pin:    Chip Select (CS) / Slave Select (SS) pin (Any GPIO)  HANDLED BY BREADBOARD
        - clock_pin: Clock (SCLK / SCK) pin (Any GPIO) 
        - data_pin:  Data input (SO / MOSI) pin (Any GPIO)
        - units:     (optional) unit of measurement to return. ("c" (default) | "k" | "f")
        - board:     (optional) pin numbering method as per RPi.GPIO library (GPIO.BCM (default) | GPIO.BOARD) HANDLED BY BREADBOARD
        '''
        #self.cs_pin = cs_pin HANDLED BY BREADBOARD
        self.clock_pin = clock_pin 
        self.data_pin = data_pin
        self.units = units
        self.data = None
        #self.board = board HANDLED BY BREADBOARD

        # Initialize needed GPIO
        #GPIO.setmode(self.board) HANDLED BY BREADBOARD
        #GPIO.setup(self.cs_pin, GPIO.OUT)
        #GPIO.setup(self.clock_pin, GPIO.OUT) HANDLED BY BREADBOARD
        GPIO.setup(self.data_pin, GPIO.IN)

        # Pull chip select high to make chip inactive
        #GPIO.output(self.cs_pin, GPIO.HIGH) HANDLED BY BREADBOARD
    ...
    def read(self):
        ...
        bytesin = 0
        # Select the chip
        #GPIO.output(self.cs_pin, GPIO.LOW) HANDLED BY BREADBOARD
        # Read in 16 bits
        for i in range(16):
